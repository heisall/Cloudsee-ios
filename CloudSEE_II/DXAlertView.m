...
#define kButtonBottomOffset 10.0f
       ...
       progressView =  [[LDProgressView alloc]initWithFrame:CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, self.alertContentLabel.center.y , contentLabelWidth, 20)];
...
- (id)initWithTitleAndProgress:(NSString *)title
{
    if (self = [super init]) {
        ...
        self.backgroundColor = [UIColor whiteColor];
        self.alertTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, kTitleYOffset, kAlertWidth, kTitleHeight)];
        self.alertTitleLabel.textAlignment = UITextAlignmentCenter;
        self.alertTitleLabel.font = [UIFont boldSystemFontOfSize:20.0f];
        self.alertTitleLabel.textColor = [UIColor colorWithRed:56.0/255.0 green:64.0/255.0 blue:71.0/255.0 alpha:1];
        [self addSubview:self.alertTitleLabel];
        ...
        progressView =  [[LDProgressView alloc]initWithFrame:CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(self.alertTitleLabel.frame)+20 , contentLabelWidth, 20)];
        progressView.color = [UIColor colorWithRed:0.00f green:0.64f blue:0.00f alpha:1.00f];
        progressView.flat = @YES;
        progressView.progress = 0.00;
        progressView.animate = @YES;
...
{
    progressView.progress = index/100.0;
}
...
{
    if (type == CustomAlertType_btn_One) {
        ...
        height = kAlertHeight -60;
        UIViewController *topVC = [self appRootViewController];
        self.frame = CGRectMake((CGRectGetWidth(topVC.view.bounds) - kAlertWidth) * 0.5, - kAlertHeight - 30, kAlertWidth, height);
        [topVC.view addSubview:self];
    }
...
- (void)removeFromSuperview
{
    ...
    UIViewController *topVC = [self appRootViewController];
    CGRect afterFrame = CGRectMake((CGRectGetWidth(topVC.view.bounds) - kAlertWidth) * 0.5, CGRectGetHeight(topVC.view.bounds), kAlertWidth, height);
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
  ...
  //  self.transform = CGAffineTransformMakeRotation(-M_1_PI / 2);
    CGRect afterFrame = CGRectMake((CGRectGetWidth(topVC.view.bounds) - kAlertWidth) * 0.5, (CGRectGetHeight(topVC.view.bounds) - kAlertHeight) * 0.5, kAlertWidth, height);
