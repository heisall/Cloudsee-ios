...
- (void)showChangeStreamView:(UIButton *)btn
{
    if (VideoStreamType_Default == nCurrentStreamType ||VideoStreamType_NoSupport == nCurrentStreamType) {
     ...
     straemView= [[JVCPopStreamView alloc] initStreamView:btn andSelectindex:nCurrentStreamType streamCountType:1];
...
- (void)setHorbarStreamView:(UIButton *)btn
{
    ...
    if(!horizonView)
    {
        horizonView = [[JVCHorizontalStreamView alloc] showHorizonStreamView:btn andSelectindex:nCurrentStreamType>0?nCurrentStreamType -1:0 streamCountType:1] ;
