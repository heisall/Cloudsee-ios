...
-(void)addNewScanDevice:(UIButton *)button
{
    ...
    if (isNewDevice) {
        ...
        [[JVCAlertHelper shareAlertHelper] alertControllerWithTitle:[NSString stringWithFormat:@"%@：%@\n(%@)",LOCALANGER(@"qrDevice"),model.strYstNumber,@"已添加"] delegate:self selectAction:@selector(addQRdevice) cancelAction:nil selectTitle:nil cancelTitle:LOCALANGER(@"关闭") alertTage:0];
